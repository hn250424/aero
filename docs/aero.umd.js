...
\r
<input id="input" type="number" />`;class a extends h{constructor(){super(p)}getInputSelector(){return"#input"}}customElements.define("aero-numeric-input",a);const g=`<style>
    :host {
...
</div>`;class u extends h{minus;plus;constructor(){super(g),this.minus=this.query("#minus"),this.plus=this.query("#plus"),this.minus.addEventListener("click",this.decrement.bind(this)),this.plus.addEventListener("click",this.increment.bind(this)),this.updateButtonBackgrondColor(this.getAttribute("button-backgroundcolor")),this.updateMinuxText(this.getAttribute("text-minus")),this.updatePlusText(this.getAttribute("text-plus")),this.updateHeight(parseInt(getComputedStyle(this).height)),new ResizeObserver(e=>{for(const i of e){const s=i.contentRect.height;this.applyStyles(`#spinbox {
...
\r
        ...
        `)}set width(t){this.setAttribute("width",t)}set height(t){this.setAttribute("height",t)}set background(t){this.setAttribute("background",t)}set color(t){this.setAttribute("color",t)}set cycle(t){this.setAttribute("cycle",t)}}customElements.define("aero-progress-spinner",l);const c=`<style>\r
    :host {\r
        position: relative;\r
    }\r
\r
