...
\r
<input id="input" type="number" />`;class u extends a{constructor(){super(g)}getInputSelector(){return"#input"}}customElements.define("aero-numeric-input",u);const m=`<style>
    :host {
...
</div>`;class l extends a{minus;plus;constructor(){super(m),this.minus=this.query("#minus"),this.plus=this.query("#plus"),this.minus.addEventListener("click",this.decrement.bind(this)),this.plus.addEventListener("click",this.increment.bind(this)),this.updateButtonBackgrondColor(this.getAttribute("button-backgroundcolor")),this.updateMinuxText(this.getAttribute("text-minus")),this.updatePlusText(this.getAttribute("text-plus")),this.updateHeight(parseInt(getComputedStyle(this).height)),new ResizeObserver(e=>{for(const i of e){const s=i.contentRect.height;this.applyStyles(`#spinbox {
                        grid-template-columns: ${s}px 1fr ${s}px;
                    }`)}}).observe(this)}getInputSelector(){return"#input"}static get observedAttributes(){return[...super.observedAttributes,"text-minus","text-plus","button-backgroundcolor"]}attributeChangedCallback(t,e,i){super.attributeChangedCallback(t,e,i),this.aeroSpinboxAttributeHandlers[t]?.(i)}aeroSpinboxAttributeHandlers={"text-minus":t=>{this.updateMinuxText(t)},"text-plus":t=>{this.updatePlusText(t)},"button-backgroundcolor":t=>{this.updateButtonBackgrondColor(t)}};updateMinuxText(t){this.minus.textContent=t||"-"}updatePlusText(t){this.plus.textContent=t||"+"}updateButtonBackgrondColor(t){this.applyStyles(`#spinbox > button { 
                background-color: ${t||"#ccc"};
            }`)}updateHeight(t){t=t||30,this.applyStyles(`#spinbox {
                grid-template-columns: ${t}px 1fr ${t}px;
            }`)}set buttonBackgroundColor(t){this.setAttribute("button-backgroundcolor",t)}set minusText(t){this.setAttribute("text-minus",t)}set plusText(t){this.setAttribute("text-plus",t)}decrement(){const t=Number(this.input.value)-Number(this.step);this.input.value=this.getValidateValue(t.toString())}increment(){let t=Number(this.input.value)+Number(this.step);this.input.value=this.getValidateValue(t.toString())}}customElements.define("aero-spinbox",l);const c=`<style>\r
    @keyframes spin {\r
        0% {\r
...
</style>`;class d extends o{constructor(){super(c),this.updateSpinnerStyles()}static get observedAttributes(){return["width","height","background","color","cycle"]}attributeChangedCallback(t,e,i){this.updateSpinnerStyles()}updateSpinnerStyles(){const t=this.getAttribute("width")||"50",e=this.getAttribute("height")||"50",i=this.getAttribute("background")||"white",s=this.getAttribute("color")||"black",n=this.getAttribute("cycle")||"1";this.applyStyles(`
            :host {
                width: ${t}px;
                height: ${e}px;
                border: 5px solid ${i};
                border-top-color: ${s};
                border-radius: 50%;
                animation: spin ${n}s linear infinite;
...
<div id="right" class="resizer horizontal"></div>`;class p extends b{constructor(){super(x)}}customElements.define("aero-resize-box",p),r.AeroNumericInput=u,r.AeroProgressSpinner=d,r.AeroResizeBox=p,r.AeroSpinbox=l,Object.defineProperty(r,Symbol.toStringTag,{value:"Module"})}));
