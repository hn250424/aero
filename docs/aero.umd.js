...
<div id="spinbox">
    ...
    <button id="plus">+</button>
</div>`;class d extends h{minus;plus;constructor(){super(c),this.minus=this.query("#minus"),this.plus=this.query("#plus"),this.minus.addEventListener("click",this.decrement.bind(this)),this.plus.addEventListener("click",this.increment.bind(this)),this.updateButtonBackgrondColor(this.getAttribute("button-backgroundcolor")),this.updateMinuxText(this.getAttribute("text-minus")),this.updatePlusText(this.getAttribute("text-plus")),this.updateHeight(parseInt(getComputedStyle(this).height)),new ResizeObserver(e=>{for(const i of e){const s=i.contentRect.height;this.applyStyles(`#spinbox {
            ...
            }`)}set buttonBackgroundColor(t){this.setAttribute("button-backgroundcolor",t)}set minusText(t){this.setAttribute("text-minus",t)}set plusText(t){this.setAttribute("text-plus",t)}decrement(){const t=Number(this.input.value)-Number(this.step);this.input.value=this.getValidateValue(t.toString())}increment(){let t=Number(this.input.value)+Number(this.step);this.input.value=this.getValidateValue(t.toString())}}customElements.define("aero-spinbox",d);class m extends n{constructor(t){super(t),this.updateContainerBackground(this.getAttribute("container-background"))}static get observedAttributes(){return["container-background"]}attributeChangedCallback(t,e,i){this.baseAeroProgressAttributeHandlers[t]?.(i)}baseAeroProgressAttributeHandlers={"container-background":t=>{this.updateContainerBackground(t)}};updateContainerBackground(t){this.applyStyles(`:host {
                background: ${t||"rgba(0, 0, 0, 0.5)"};
            }`)}set containerBackground(t){this.setAttribute("container-background",t)}}const b=`<style>\r
    :host {\r
        display: flex;\r
...
<div id="right" class="resizer horizontal"></div>`;class p extends x{constructor(){super(z)}}customElements.define("aero-resize-box",p),r.AeroNumericInput=u,r.AeroProgressSpinner=l,r.AeroResizeBox=p,r.AeroSpinbox=d,Object.defineProperty(r,Symbol.toStringTag,{value:"Module"})}));
