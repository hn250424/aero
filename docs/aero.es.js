...
class a extends o {
  ...
  _decimalPlaces;
  constructor(t) {
    super(t), this.initializeInput(), this.updateMinValue(this.getAttribute("min")), this.updateMaxValue(this.getAttribute("max")), this.updateStepValue(this.getAttribute("step"));
    const e = this.getAttribute("value") ?? "0";
    this._input.value = e;
  }
  initializeInput() {
    this._input = this.query(this.getInputSelector()), this._input.addEventListener("focusout", () => {
      const t = this.getValidateValue(this._input.value);
      this._input.value = t;
    });
  }
  getValidateValue(t) {
...
class m extends o {
    ...
    super(t), this._topResizer = this.query("#top"), this._bottomResizer = this.query("#bottom"), this._leftResizer = this.query("#left"), this._rightResizer = this.query("#right"), this.updateMinWidthValue(this.getAttribute("min-width")), this.updateMaxWidthValue(this.getAttribute("max-width")), this.updateMinHeightValue(this.getAttribute("min-height")), this.updateMaxHeightValue(this.getAttribute("max-height")), this.updateTopResizerState(this.hasAttribute("resize-top")), this.updateBottomResizerState(this.hasAttribute("resize-bottom")), this.updateLeftResizerState(this.hasAttribute("resize-left")), this.updateRightResizerState(this.hasAttribute("resize-right")), document.addEventListener("mousemove", (e) => {
      this.isDragging && (this.animationFrameId && cancelAnimationFrame(this.animationFrameId), this.animationFrameId = requestAnimationFrame(() => {
        ...
        if (this.isTopDragging) {
          const s = i.bottom - e.clientY, n = Math.min(Math.max(s, this._nMinHeight), this._nMaxHeight);
          this.style.height = `${n}px`;
        } else if (this.isBottomDragging) {
          const s = e.clientY - i.top, n = Math.min(Math.max(s, this._nMinHeight), this._nMaxHeight);
          this.style.height = `${n}px`;
        } else if (this.isLeftDragging) {
          const s = i.right - e.clientX, n = Math.min(Math.max(s, this._nMinWidth), this._nMaxWidth);
          this.style.width = `${n}px`;
        } else if (this.isRightDragging) {
          const s = e.clientX - i.left, n = Math.min(Math.max(s, this._nMinWidth), this._nMaxWidth);
          this.style.width = `${n}px`;
        }
      }));
  ...
  processMousedownEvent = (t, e) => {
    switch (t.preventDefault(), document.body.style.userSelect = "none", this.isDragging = !0, e) {
      case "top":
