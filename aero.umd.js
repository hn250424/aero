...
`;class _ extends p{constructor(){super(w)}getInputSelector(){return"#input"}}customElements.define("aero-numeric-input",_);const y=`<style>\r
	:host {\r
		border: 1px solid #ccc;\r
		display: block;\r
\r
...
`;class m extends s{_size;_thickness;_radius;_circumference;_trackColor;_arcColor;_cycle;_arcRatio;_$svg;_$track;_$arc;constructor(){super($),this._syncHostAttributes(),this._buildSvg(),this._syncSvgAttributes(),this._syncStyles()}_buildSvg(){const t="http://www.w3.org/2000/svg";this._$svg=document.createElementNS(t,"svg"),this._$track=document.createElementNS(t,"circle"),this._$arc=document.createElementNS(t,"circle"),this._$track.classList.add("track"),this._$arc.classList.add("arc"),this._$svg.appendChild(this._$track),this._$svg.appendChild(this._$arc),this.shadow.appendChild(this._$svg)}static get observedAttributes(){return["size","thickness","track-color","arc-color","cycle","arc-ratio"]}attributeChangedCallback(t,e,i){this._syncHostAttributes(),this._syncSvgAttributes(),this._syncStyles()}_syncHostAttributes(){this._size=parseInt(this.getAttribute("size")||"50"),this._thickness=parseInt(this.getAttribute("thickness")||"4"),this._radius=this._size/2-this._thickness-1,this._circumference=2*Math.PI*this._radius,this._trackColor=this.getAttribute("track-color")||"transparent",this._arcColor=this.getAttribute("arc-color")||"black",this._cycle=parseInt(this.getAttribute("cycle")||"2"),this._arcRatio=parseFloat(this.getAttribute("arc-ratio")||"90")/100}_syncSvgAttributes(){this._$svg.setAttribute("viewBox",`0 0 ${this._size} ${this._size}`),this._$svg.setAttribute("width",String(this._size)),this._$svg.setAttribute("height",String(this._size)),this._$track.setAttribute("cx",String(this._size/2)),this._$track.setAttribute("cy",String(this._size/2)),this._$track.setAttribute("r",String(this._radius)),this._$arc.setAttribute("cx",String(this._size/2)),this._$arc.setAttribute("cy",String(this._size/2)),this._$arc.setAttribute("r",String(this._radius))}_syncStyles(){this.applyStyles(`
		...
		`)}}customElements.define("aero-indeterminate-spinner",m);const k=`<style>\r
	:host {\r
		position: relative;\r
		display: block;\r
		width: 300px;\r
...
\r
<span id="text"></span>\r
`,E={top:"90%",left:"50%",ms:3e3,background:"black",color:"white"};class l extends s{_$text;constructor(t,e){super(A);const{top:i,left:n,ms:o,background:a,color:u}=e;this._$text=this.query("#text"),this._$text.textContent=t,this.applyStyles(`
			:host {
				top: ${i};
				left: ${n};
		...
		`),document.body.appendChild(this),this.addEventListener("animationend",()=>{this.remove()},{once:!0})}static show(t,e={}){const i={...E,...e};new l(t,i)}}customElements.define("aero-toast",l);const C=`<style>\r
	:host {\r
		position: fixed;\r
		top: 0;\r
		left: 0;\r
...
`,S={fontSize:"1rem",containerBorder:"1px solid lightgrey",containerBoxShadow:"0 4px 8px rgba(0, 0, 0, 0.2)",primaryBackgroundColor:`${{blue_5:"#2563eb"}.blue_5}`,primaryColor:"white",secondaryBackgroundColor:"grey",secondaryColor:"white",buttonBorderRadius:"0"};class c extends s{_$message;_$ok;_$cancel;_resolve;_handleKeyDown;constructor(t,e,i){super(t);const{fontSize:n,containerBorder:o,containerBoxShadow:a,primaryBackgroundColor:u,primaryColor:M,secondaryBackgroundColor:L,secondaryColor:D,buttonBorderRadius:R}=i;this._$message=this.query("#message"),this._$message.textContent=e,this._$ok=this.query("#ok"),this._$cancel=this.queryOptional("#cancel"),this.applyStyles(`
			#container {
				font-size: ${n};
				border: ${o};
				box-shadow: ${a};
		...
		`),this._$ok.onclick=()=>{this.remove(),this._resolve?.(!0),this._resolve=void 0},this._$cancel&&(this._$cancel.onclick=()=>{this.remove(),this._resolve?.(!1),this._resolve=void 0}),this._handleKeyDown=x=>{x.key==="Enter"?this._$ok.click():x.key==="Escape"&&(this._$cancel?this._$cancel.click():this._$ok.click())},window.addEventListener("keydown",this._handleKeyDown),document.body.appendChild(this)}static alert(t,e={}){return this._create(C,t,e)}static confirm(t,e={}){return this._create(I,t,e)}static _create(t,e,i){const n={...S,...i};return new Promise(o=>{const a=new c(t,e,n);a._resolve=o})}}customElements.define("aero-popup",c);const H=`<style>\r
\r
</style>\r
\r
<div id="list"></div>\r
<div class="highlight"></div>\r
`;class f extends s{_items=[];_$list;_itemHeight=0;_visibleCount=5;_maxHeight=0;_index=0;_y=0;_startY=0;_isDown=!1;_onPointerDown=t=>{this._isDown=!0,this._startY=t.pageY,this._$list.style.transition="none",window.addEventListener("pointermove",this._onPointerMove),window.addEventListener("pointerup",this._onPointerUp)};_onPointerMove=t=>{if(!this._isDown)return;const e=t.pageY-this._startY;this._startY=t.pageY;const i=this._y+e,n=Math.max(this._maxHeight,Math.min(0,i));this._move(n)};_onPointerUp=()=>{this._isDown&&(this._isDown=!1,window.removeEventListener("pointermove",this._onPointerMove),window.removeEventListener("pointerup",this._onPointerUp),this._end())};_wheelTimer;_onWheel=t=>{t.preventDefault();const e=this._y-t.deltaY,i=Math.max(this._maxHeight,Math.min(0,e));this._move(i),clearTimeout(this._wheelTimer),this._wheelTimer=window.setTimeout(()=>{this._end()},100)};constructor(){super(H),this._$list=this.query("#list"),this._itemHeight=parseInt(this.getAttribute("item-height")??"30"),this._visibleCount=parseInt(this.getAttribute("visible-count")??"5"),this._syncStyles()}connectedCallback(){this.addEventListener("pointerdown",this._onPointerDown),this.addEventListener("wheel",this._onWheel,{passive:!1})}disconnectedCallback(){this.removeEventListener("pointerdown",this._onPointerDown),this.removeEventListener("wheel",this._onWheel)}static get observedAttributes(){return["item-height","visible-count"]}attributeChangedCallback(t,e,i){this._aeroRollerAttributeHandlers[t]?.(i)}_aeroRollerAttributeHandlers={"item-height":t=>{this._updateItemHeight(parseInt(t??"30"))},"visible-count":t=>{this._updateVisibleCount(parseInt(t??"5"))}};setItems(t){this._items=t,this._updateMaxHeight(),this._render(),this._reset()}_updateItemHeight(t){this._itemHeight=t,this._updateMaxHeight(),this._syncStyles(),this.scrollToIndex(this._index)}_updateVisibleCount(t){t<0&&(this._visibleCount=0),this._visibleCount=t%2===0?t+1:t,this._syncStyles(),this._render(),this.scrollToIndex(this._index)}_updateMaxHeight(){const t=Math.max(0,this._items.length-1);this._maxHeight=-t*this._itemHeight}_syncStyles(){this.applyStyles(`
			* {
				margin: 0;
				padding: 0;
		...
		`)}_render(){const t=Math.floor(this._visibleCount/2),e=Array(t).fill('<div class="item"></div>').join("");this._$list.innerHTML=e+this._items.map(i=>`<div class="item">${i}</div>`).join("")+e}_reset(){this._index=0,this._move(0,!0)}get index(){return this._index}scrollToIndex(t){const e=Math.max(0,this._items.length-1),i=Math.max(0,Math.min(t,e));this._index=i;const n=-(i*this._itemHeight);this._$list.style.transition="transform 0.2s ease-out",this._move(n,!0),setTimeout(()=>{this._$list.style.transition="none"},200)}get current(){return this._items[this._index]}_move(t,e=!1){this._y=t,e?this._$list.style.transition="none":this._$list.style.transition="transform 0.2s ease-out",this._$list.style.transform=`translateY(${this._y}px)`}_end(){const t=Math.round(Math.abs(this._y/this._itemHeight));this.scrollToIndex(t),this.dispatchEvent(new CustomEvent("change",{detail:{index:t,value:this._items[t]}}))}}customElements.define("aero-roller",f),r.AeroIndeterminateSpinner=m,r.AeroNumericInput=_,r.AeroOption=v,r.AeroPopup=c,r.AeroResizableBox=d,r.AeroRoller=f,r.AeroSelect=b,r.AeroSpinbox=g,r.AeroToast=l,Object.defineProperty(r,Symbol.toStringTag,{value:"Module"})}));
